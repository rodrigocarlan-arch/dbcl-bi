...
function drawCadastros(){
  ...
  document.getElementById('cadastro-content').innerHTML=`<div class="note"><b>Edição: ${esc(s.owner)} · ${s.local_only===false?'online':'neste computador'}.</b> ${s.enabled?'O cadastro do BI é a fonte de pessoas e tarifas.':'Importação disponível para conferência; ativação do cálculo pendente.'} ${s.pending?'<b>Há cadastros ainda não refletidos na base exibida.</b>':'A base exibida corresponde à revisão salva.'}</div>
  ...
  document.getElementById('admin-backup').onclick=async()=>{try{const r=await adminAPI('backup',{});if(r.backup){const url=URL.createObjectURL(new Blob([JSON.stringify(r.backup)],{type:'application/json'}));const a=document.createElement('a');a.href=url;a.download=r.filename||'dbcl-cadastros.json';document.body.append(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);}toast(r.message);}catch(e){toast(e.message);}};
